# ============================================================
# Butler Robot - Milestone 2
#
# Flow:
# Order -> Kitchen -> Kitchen Confirm
#       -> Table -> Table Timeout -> Home
#
# States:
# IDLE
# GOING_TO_KITCHEN
...
class ButlerRobot(Node):
    def __init__(self):
        ...
        self.get_logger().info("Butler Robot Ready!")
    ...
    def order_callback(self, msg):
        ...
        self.get_logger().info(
            f"Order received for {self.current_table}"
        )
    ...
    def result_callback(self, future):
        ...
        if result.status != GoalStatus.STATUS_SUCCEEDED:
            self.get_logger().error(
                f"Failed to reach {self.destination}"
            )
            self.state = "IDLE"
            return
        self.get_logger().info(
            f"Reached {self.destination}"
        )
